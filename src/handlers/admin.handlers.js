  ...
  addLiterature: async (ctx) => {
    try {
      ...
      if (!ctx.session.adminAction) {
        ...
        return ctx.reply("Qaysi bo'limga ma'lumot qo'shmoqchisiz?", {
          reply_markup: keyboard,
        });
      }

      if (ctx.session.adminAction === "waitingForSection") {
        const section = Object.entries(LITERATURE_SECTIONS).find(
          ([_, value]) => value === ctx.message.text
        )?.[0];

        if (!section) {
          return ctx.reply(
            "Noto'g'ri bo'lim tanlandi. Iltimos qaytadan urinib ko'ring."
          );
        }
      ...
      if (ctx.session.adminAction === "waitingForFile") {
        ...
        if (!caption || !caption.includes("\n")) {
          return ctx.reply(
            "Iltimos, sarlavha va tavsifni to'g'ri formatda yuboring"
          );
        }
  ...
  deleteLiterature: async (ctx) => {
    try {
      ...
      if (literatureId < 0 || literatureId >= literature.length) {
        return ctx.reply(
          "Noto'g'ri ID raqami. Iltimos qaytadan urinib ko'ring."
        );
      }
